...
phone_review_classifier = models.PhoneReviewsToneSentimentClassifier()
...
class DemoModel(FlaskForm):
    ...
    phone_tone = TextAreaField('Текст для анализа тональности', validators=[Length(min=0, max=250)])
...
def render_main():
    ...
    if request.method == "POST":
        ...
        # for phone reviews analysis
        user_text_phone = form.phone_tone.data
        if user_text_phone != '':
            predictions_phone = phone_review_classifier.get_prediction_message(user_text_phone).split(',')
            if predictions_phone[1] == 'positive':  # class 1 - positive feedback
                class_res_phone = 'POSITIVE FEEDBACK'
                persent_phone = predictions_phone[2]
            elif predictions_phone[1] == 'negative':
                class_res_phone = 'NEGATIVE FEEDBACK'
                persent_phone = str(100 - float(predictions_phone[2]))
            else:
                class_res_phone = 'NEUTRAL'
                persent_phone = predictions_phone[2]
            prediction_message_phone = 'Ваш отзыв классифицирован как: {} c вероятностью: {} %'.format(class_res_phone,
                                                                                                       persent_phone)
        else:
            prediction_message_phone = None

        return render_template('demo.html', form=form, result=prediction_message, result_tone=prediction_message_tone,
                               result_phone=prediction_message_phone)
    else:
        return render_template('demo.html', form=form, result=None, result_tone=None, result_phone=None)
